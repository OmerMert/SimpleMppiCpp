@echo off
REM ============================================================
REM  Phase 2: PhysX (Leatherback) Isaac Sim <-> C++ MPPI bridge.
REM  Launches MppiCpp.exe automatically (no need to run it separately).
REM
REM  Usage:
REM    run_physx.bat                       (uses REF_PATH_FILE from config.json)
REM    run_physx.bat --headless            (no GUI window)
REM    run_physx.bat data\path_sine.csv    (override the path)
REM ============================================================
setlocal
cd /d "%~dp0"
...
call "%ISAAC_PY%" "%~dp0isaacsim_bridge_physx.py" %*
endlocal
